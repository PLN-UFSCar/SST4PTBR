...
# def gerar_texto_com_lmstudio(prompt):
#     """
#     Gera texto reescrito usando um modelo carregado no LM Studio.
#     Requer que o servidor LM Studio esteja rodando e o modelo especificado carregado.
#     O nome do modelo é hard-coded aqui.
#     """
#     model_name = "meta-llama-3-8b-instruct"
#     try:
#         with lms.Client() as client:
#             print(f"Gerando texto com LM Studio ({model_name})...")
#             modelo = client.llm.model(model_name)

#             #resposta_raw = modelo.respond(prompt, temperature=0.3, max_tokens=1000)
#             resposta_raw = modelo.respond(prompt)
#             return str(resposta_raw) # Garante que estamos retornando uma string

#     except Exception as e:
#         print(f"Erro ao conectar ou usar o LM Studio: {e}")
#         print("Certifique-se de que o servidor LM Studio esteja rodando e o modelo esteja carregado.")
#         if "identifier not found" in str(e).lower() or "not loaded" in str(e).lower():
#              print(f"ATENÇÃO: O modelo '{model_name}' não foi encontrado ou não está ativamente carregado no LM Studio.")
#              print("Por favor, selecione e carregue o modelo na aba 'Local Server' do LM Studio e reinicie o servidor.")
#         return None